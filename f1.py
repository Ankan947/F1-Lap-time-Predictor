...
# ==========================================
print("Loading data...")
# Load datasets (make sure these CSVs are in the same folder)
laps = pd.read_csv('lap_times.csv')
pit_stops = pd.read_csv('pit_stops.csv')
results = pd.read_csv('results.csv')

# Select 2019 Italian GP (raceId = 1025) - Fast, dry track, no red flags
TARGET_RACE_ID = 1025
...
# Remove lap 1 (standing start is always slow) and pit stop laps (outliers)
# Also remove excessively slow laps (e.g., virtual safety car or traffic, say > 90 seconds for Monza)
...
# (This fulfills the "stint-based split, not random" requirement)
...
# Train Tire Age Model (Random Forest usually captures degradation well)
...
# ==========================================
# Pick one driver from the test set to plot their stint (e.g., the race winner)
plot_driver_id = driver_ids[0] 
